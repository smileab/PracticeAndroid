package com.smileab.mobile.practice.widget;
...
public class StarBar extends View {
    ...
    public void setStarMark(float mark) {
        ...
        } else if (currentMode == Mode.FLOAT) {
            starMark = Math.round(mark * 10) * 1.0f / 10;
        } else if (currentMode == Mode.HALF) {
            if (mark + 0.5 > Math.ceil(mark)) {
                starMark = (float) Math.ceil(mark);
            } else if (mark + 0.2 < (Math.ceil(mark) - 0.5f)) {
                starMark = (float) Math.floor(mark);
            } else {
                starMark = (float) (Math.floor(mark) + 0.5f);
            }
